...
class GoogleCloudStorageTransfer:
  ...
  def GcstApi(self) -> 'googleapiclient.discovery.Resource':
    ...
    self.gcst_api_client = common.CreateService(
        'storagetransfer', self.CLOUD_STORAGE_API_VERSION)
    return self.gcst_api_client
  ...
  def S3ToGCS(self,
    ...
    """
    aws_creds = account.AWSAccount(zone).session.get_credentials(zone)
    transfer_job_body = {
        'transferSpec': {
            'awsS3DataSource': {
                'bucketName': s3_path,
    ...
    request = gcst_jobs.create(body=transfer_job_body)
    response = request.execute()
    return response  # type: Dict[str, Any]
